#!/usr/bin/env python3
"""
Special wrapper script for 160×160×80mm configuration with multi-color pores,
advanced analysis v3, and custom colorbar using pore colors.
"""
...
print("Applying dimension override (160×160×80mm) with multi-color pores, advanced analysis v3, and custom colorbar...")

# Store original function
original_load_default_config = MaterialConfig._load_default_config


def custom_160x160x80_advanced3_config(self):
    # First call original to get all default settings
    original_load_default_config(self)

    # Apply 160×160×80mm dimensions
    self.board_length_mm = 160.0
    self.board_width_mm = 160.0
    self.board_thickness_mm = 80.0

    # Update coordinate scaling
    self.length_scale = 2.0
    self.width_scale = 2.0
    self.thickness_scale = 1.0
    self.aspect_ratio = [2, 2, 1]

    # Adjust visualization limits
    self.x_limits = (-2.0, 2.0)
    self.y_limits = (-2.0, 2.0)
    self.z_limits = (-1.0, 1.0)

    # Matrix boundaries
    self.matrix_fill_x_bounds = (-1.8, 1.8)
    self.matrix_fill_y_bounds = (-1.8, 1.8)
    self.matrix_fill_z_bounds = (-0.8, 0.8)

    # Particle distribution
    self.matrix_length_norm = 1.8
    self.matrix_width_norm = 1.8
    self.default_x_bounds = (-1.8, 1.8)
    self.default_y_bounds = (-1.8, 1.8)
    self.default_z_bounds = (-0.8, 0.8)

    # Camera and view settings
    self.camera_position = np.array([2.5, 2.5, 1.5])
    self.view_elevation = 20
    self.view_azimuth = 45

    # Scale pore counts based on volume
    default_volume = 160 * 160 * 40
    new_volume = 160 * 160 * 80
    ...
    self.show_legends = False  # Disable legends for advanced3
    ...
    print("\nSimple Pore Analysis - Advanced v3 with Multi-Color Pores and Custom Colorbar")
    print("=============================================================================")
    print("160×160×80mm board configuration applied:")
    print(
...
MaterialConfig._load_default_config = custom_160x160x80_advanced3_config
...
print(
    f"[DEBUG] Advanced analysis v3 enabled: {getattr(CONFIG, 'enable_advanced_analysis', False)}")
...
try:
    ...
    print("[DEBUG] Manually generating advanced analysis v3 files...")
    ...
    if data is not None:
        ...
        for diam, intr, sample_name in samples:
            output_file = f"out/{sample_name}_advanced_v3_analysis.png"
            print(f"[DEBUG] Creating advanced analysis v3 for {sample_name}")
            print(
            ...
            try:
                ...
                print(
                    f"[DEBUG] Starting advanced analysis v3 rendering for {sample_name}...")
                ...
                print(
                    f"[DEBUG] Advanced analysis v3 for {sample_name} timed out, skipping")
                continue
            except Exception as rendering_error:
                print(
                    f"[DEBUG] Advanced analysis v3 rendering failed for {sample_name}: {rendering_error}")
                continue
            finally:
                signal.alarm(0)

            print(f"[DEBUG] Advanced analysis v3 saved: {output_file}")
    else:
        print("[DEBUG] Could not load data for advanced analysis v3")

except Exception as e:
    print(f"[DEBUG] Failed to create advanced analysis v3 files: {e}")
    import traceback
    traceback.print_exc()
